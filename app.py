...
class SystemPrompt(db.Model):
    ...
    llm_model = db.Column(db.String(20), nullable=False, default='gemini')  # 使用するLLMモデル
    created_at = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, nullable=False, default=datetime.utcnow, onupdate=datetime.utcnow)
...
def get_system_prompt(model_name):
    ...
    try:
        ...
        if system_prompt:
            ...
            return system_prompt.prompt, system_prompt.llm_model
        else:
            print(f"No system prompt found for {model_name} in database")
            # デフォルトのモデルの場合
            if model_name in default_llm_models:
                return SYSTEM_PROMPT, default_llm_models[model_name]
            else:
                # その他のモデルの場合はデフォルト値を返す
                return "システムプロンプトが設定されていません。", "gemini"
    except Exception as e:
        print(f"Error getting system prompt: {str(e)}")
        return "システムプロンプトの取得中にエラーが発生しました。", "gemini"
...
model_functions = {
    ...
    "要件定義書のヒアリングAI": agent.要件定義書のヒアリングAI,
    "ビジネス向け要件定義書": agent.ビジネス向け要件定義書,
    "エンジニア向け要件定義書": agent.エンジニア向け要件定義書,
    "金額提示相談AI": agent.金額提示相談AI,
}
...
# システムプロンプトの更新API
@app.route("/admin/system_prompts/<model_name>/llm_model", methods=["PUT"])
@login_required
@admin_required
def update_llm_model(model_name):
    ...
    if not llm_model:
        return jsonify({"error": "LLMモデルは必須です"}), 400
    ...
    if system_prompt:
        ...
        try:
            db.session.commit()
            return jsonify({"success": True, "message": "LLMモデルを更新しました"})
        except Exception as e:
            db.session.rollback()
            return jsonify({"error": str(e)}), 500
    else:
        return jsonify({"error": "システムプロンプトが見つかりません"}), 404
    ...
    try:
        ...
        if has_image:
            ...
            if user_input.startswith("@web"):
                ...
                # システムプロンプトとLLMモデルを取得
                system_prompt, llm_model = get_system_prompt(model)
                
                # 基本モデル（groq, gpt, gemini, claude, xai）の場合
                if model in ['groq', 'gpt', 'gemini', 'claude', 'xai']:
                    assistant_message_content = model_functions[model](messages_list)
                else:
                    # カスタムモデル（VOLTMIND AI, 税務GPTなど）の場合
                    assistant_message_content = model_functions[model](messages_list, system_prompt, llm_model)
            ...
            if existing_prompts_count == 0:
                ...
                for model_name, prompt_text in DEFAULT_SYSTEM_PROMPTS.items():
                    new_prompt = SystemPrompt(
                        model_name=model_name,
                        prompt=prompt_text,
                        llm_model='gemini'  # デフォルトのLLMモデルを設定
                    )
